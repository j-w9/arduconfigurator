#!/usr/bin/env node
// Generates apps/web/public/models/sub.gltf — a small, self-authored
// underwater-ROV model for the Setup craft preview.
//
// The other models in that folder are copied from Betaflight Configurator
// (GPL-3.0, multirotors only). ArduSub has no ROV there, so this emits a
// real glTF 2.0 asset (boxes for hull / buoyancy block / side rails /
// thruster pods) from a deterministic script rather than vendoring an
// opaque binary. Re-run with `node scripts/generate-sub-gltf.mjs`.
//
...
const outPath = join(here, '..', 'apps', 'web', 'public', 'models', 'sub.gltf')

// [centerX, centerY, centerZ, halfX, halfY, halfZ]
const BOXES = [
  [0, 0, 0, 0.55, 0.46, 1.55], // pressure hull
  [0, 0.58, 0, 0.8, 0.18, 1.78], // buoyancy block
  [-0.78, 0, 0, 0.05, 0.34, 1.45], // left frame rail
  [0.78, 0, 0, 0.05, 0.34, 1.45], // right frame rail
  [-0.66, -0.04, 1.55, 0.2, 0.2, 0.32], // rear-left thruster
  [0.66, -0.04, 1.55, 0.2, 0.2, 0.32], // rear-right thruster
  [-0.92, 0.06, -0.55, 0.2, 0.2, 0.3], // forward-left vectored thruster
  [0.92, 0.06, -0.55, 0.2, 0.2, 0.3], // forward-right vectored thruster
  [-0.42, 0.42, 0.1, 0.16, 0.12, 0.16], // left vertical thruster
  [0.42, 0.42, 0.1, 0.16, 0.12, 0.16] // right vertical thruster
]
...
const gltf = {
  asset: { version: '2.0', generator: 'arduconfigurator scripts/generate-sub-gltf.mjs' },
  scene: 0,
  scenes: [{ name: 'Scene', nodes: [0] }],
  nodes: [{ name: 'sub', mesh: 0 }],
  meshes: [
    {
      name: 'sub',
  ...
  materials: [
    {
      ...
      pbrMetallicRoughness: {
        baseColorFactor: [0.45, 0.72, 0.8, 1],
        metallicFactor: 0.22,
