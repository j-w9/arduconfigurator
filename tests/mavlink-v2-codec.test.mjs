...
test('MavlinkV2Codec decodes SCALED_IMU temperature (msgid 26, crc_extra 170)', () => {
  // A real SCALED_IMU v2 frame generated by pymavlink (ardupilotmega dialect):
  // time_boot_ms=12345, acc/gyro/mag set, temperature=3756 cdeg (37.56 C).
  const frame = Uint8Array.from(
    Buffer.from('fd1800000001011a0000393000000a0014001e000100020003006400c8002c01ac0e6a50', 'hex')
  )
  const decoder = new MavlinkV2Codec()
  const decoded = decoder.push(frame)
  const msg = decoded.map((e) => e.message).find((m) => m && m.type === 'SCALED_IMU')
  assert.ok(msg, 'SCALED_IMU decoded')
  assert.equal(msg.timeBootMs, 12345)
