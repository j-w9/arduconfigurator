...
function buildSerialPortParameterDefinitions(maxPortNumber: number): FirmwareMetadataBundle['parameters'] {
  ...
  for (let portNumber = 0; portNumber <= maxPortNumber; portNumber += 1) {
    ...
    definitions[`SERIAL${portNumber}_BAUD`] = {
      ...
      category: 'ports',
      minimum: 1,
      // 12.5 MBaud is encoded as the direct rate 12500000 (see
      // ARDUCOPTER_SERIAL_BAUD_LABELS) — the ceiling must allow it or selecting
      // the top baud option reads back as invalid.
      maximum: 12500000,
